...
namespace picosha256 {
namespace detail {

inline uint32_t mask_sat(uint32_t val) { return val & 0xffffffff; }
inline uint32_t rotr(uint32_t x, uint32_t n) { return mask_sat((x >> n) | (x << (32 - n))); }
...
inline std::string hash256_hex_string(const std::string& src) {
    ...
    for (size_t chunk = 0; chunk < data.size(); chunk += 64) {
        ...
        for (int i = 16; i < 64; ++i) {
            w[i] = detail::sub1(w[i - 2]) + w[i - 7] + detail::sub0(w[i - 15]) + w[i - 16];
        }
        ...
        for (int i = 0; i < 64; ++i) {
            uint32_t temp1 = h_val + detail::sig1(e) + detail::choose(e, f, g) + K[i] + w[i];
            uint32_t temp2 = detail::sig0(a) + detail::majority(a, b, c);
